...
// Using direct export URL format - more reliable
    ...
    res.on('end', () => {
        try {
            ...
            if (data.includes('<HTML>') || data.includes('<html>') || data.includes('Temporary Redirect')) {
                console.error('❌ Received HTML redirect instead of CSV data!');
                console.error('🔧 Google Sheets is redirecting the request');
